...
class ExerciseController<Req extends TypeHttp["Request"], Res extends TypeHttp["Response"]>{
    constructor() {
    }
    async createCategory(req:Req, res:Res) {
        ...
        if (!req.body.data) throw ErrorTypes.ValidationError("category data is required");
    ...
    async therapistGetsAllCategories(req:Req, res:Res) {
        //@ts-ignore
        const user = req.payload // decodes token for therapist user.id
        assertRole(user.role).isTherapist()
    ...
    async createExercise(req:Req, res:Res) {
        ...
        if (!req.body.data) throw ErrorTypes.ValidationError("exercise data is required");
    ...
    async therapistGetsAllExercises(req:Req, res:Res) {
        //@ts-ignore
    ...
    async createPlan(req:Req, res:Res) {
        ...
        if(!req.params["patient_id"]) throw ErrorTypes.UnauthorizedAccess("patient.id is required")
    ...
    async getPlanByIdAndPatientId(req:Req, res:Res) {
        console.log("getPlanByIdAndPatientId")
        //@ts-ignore
        const user = req.payload // decodes token for therapist/patient user.id
        if (user.role === "company") throw ErrorTypes.ServerError("Company sees plans edge case, not implemented")
        ...
        console.log(user)
        console.log(patient_id, "pla", plan_id)
        ...
        if (user.role === "therapist") {
            ...
            console.log("Got HErE, therapist")
