...
class AuthController<Req extends TypeHttp["Request"], Res extends TypeHttp["Response"]>{
    constructor() {
    }
    ...
    async changePassword(req:Req, res:Res) {
     ...
     if (!oldPassword || !newPassword) throw ErrorTypes.ValidationError('Old password and new password are required');
    ...
    async recoverPassword(req:Req, res:Res) {
      //@ts-ignore
      ...
      if (!email || !recoveryToken) {
        ...
        try {
          ...
          console.log(url)
          if (!req.body.email) throw ErrorTypes.ValidationError('Email is required');
          logger.route(gateway, email)
          req.body.url = url
          const response = await axios.post(gateway, req.body)
          res.status(response.status).send(response.data)
        ...
        try {
          ...
          if (!req.body.newPassword) throw ErrorTypes.ValidationError('newPassword is required');
          logger.route(gateway, email)
          const response = await axios.post(gateway, req.body)
          res.status(response.status).send(response.data)
