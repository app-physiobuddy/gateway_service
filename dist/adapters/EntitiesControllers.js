...
class EntitiesController {
    ...
    async addTherapist(req, res) {
        ...
        if (!req.body.therapistEmail)
            throw ErrorTypes.NotFoundError("therapistEmail is required");
    ...
    async therapistAddsPatient(req, res) {
        //@ts-ignore
        const user = req.payload; // decodes token for therapist user.id
        ...
        if (!req.body.patientEmail)
            throw ErrorTypes.UnauthorizedAccess("therapistEmail is required");
        req.body.data = {
    ...
    async getAllPatientsByTherapistId(req, res) {
        //@ts-ignore
        const user = req.payload; // decodes token for therapist user.id
        assertRole(user.role).isTherapist();
        const user_id = Number(req.params.user_id);
    ...
    async therapistGetsPatientByPatientId(req, res) {
        //@ts-ignore
        const user = req.payload; // decodes token for therapist user.id
        assertRole(user.role).isTherapist();
        const user_id = Number(req.params.user_id);
    ...
    async patientGetsPatientsByUserId(req, res) {
        //@ts-ignore
        const user = req.payload; // decodes token for therapist user.id
